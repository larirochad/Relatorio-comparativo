...
def normalizar_LC03(df: pd.DataFrame) -> dict:
    try:
        #print("[normalizar_LC03] Iniciando normalização LC03...")
        df = df.copy()
        ...
        #print(f"[normalizar_LC03] Tipos antes do mapeamento (amostra): {tipos_antes[:12]}")

        # Aplica mapeamento específico para TM-08 (principalmente GTIGN/GTIGF e variações em português)
        df_mapeado = mapear_eventos_tipo_mensagem_LC03(df)

        tipos_depois = df_mapeado[coluna_tipo].dropna().astype(str).unique().tolist()
        #print(f"[normalizar_LC03] Tipos após mapeamento (amostra): {tipos_depois[:12]}")
        ...
        #print(f"[normalizar_LC03] Contagens -> 667 (Ignição ligada): {cont_667}, 668 (Ignição desligada): {cont_668}")
        ...
        #print("[normalizar_LC03] Exemplo de legendas:")
        print(df_mapeado[['Tipo Mensagem Legenda']].head(5).to_string(index=False))

        #print("[normalizar_LC03] Normalização LC03 concluída.")
        return df_mapeado
...
def mapear_eventos_tipo_mensagem_LC03(df: pd.DataFrame) -> pd.DataFrame:
    """
    ...
    def mapear_linha(idx: int, tipo_raw):
        ...
        if tipo_upper == 'IGN' or 'IGINI' in tipo_upper or 'IGNIÇÃO LIGADA' in tipo_upper:
            return 667
        if tipo_upper == 'IGF' or 'DESLIGADA' in tipo_upper or 'IGNIÇÃO DESLIGADA' in tipo_upper:
            return 668
...
if __name__ == "__main__":
    ...
    df_mapeado = mapear_eventos_tipo_mensagem_LC03(df_exemplo)
